...
const AIConsultant: React.FC<AIConsultantProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl h-[80vh] bg-zinc-950 border border-white/10 rounded-[2.5rem] flex flex-col shadow-2xl animate-in zoom-in duration-300">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-10 py-10 space-y-8">
          {messages.map((m, i) => (
        ...
        <div className="p-8 border-t border-white/5">
          <div className="relative flex items-center">
            <input 
              ...
              className="w-full bg-zinc-900/50 border border-white/10 rounded-full px-8 py-4 pr-16 focus:outline-none focus:border-white/20 transition-all text-sm"
            />
            <button 
              onClick={handleSend}
              disabled={isLoading || !input.trim()}
              className="absolute right-2 p-3 bg-white text-black rounded-full hover:scale-105 active:scale-95 transition-all disabled:opacity-20"
            >
